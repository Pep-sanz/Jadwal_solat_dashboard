...
import { CreateTextMarquee } from './create-text-marquee';
import {
  useDeleteTextMarquues,
  useListTextMarquees
} from '@/hooks/use-text-marquees';
import { UpdateMarquee } from './update-text-marquee';

export default function TextMarqueeList() {
  ...
  const { data, isFetching } = useListTextMarquees({
    mosque: params.mosqueId as string
  });
  const { mutate, status } = useDeleteTextMarquues({
    mosque: params.mosqueId as string
  });
    ...
    <Card className="w-full">
      <CardHeader className="flex flex-row justify-between items-center">
        <div className="space-y-2">
          <CardTitle>Daftar Text Marquee</CardTitle>
          <CardDescription>Daftar Text Marquee </CardDescription>
        </div>
        <Button
          onClick={() => {
            setDialog((prev) => ({ ...prev, create: true }));
